...
({ unsigned long _tmp__; \
	__asm__ volatile("inl %%dx, %0" : "=a" (_tmp__) : "d" ((unsigned short)(port))); \
	_tmp__; })
...
({ unsigned short _tmp__; \
	__asm__ volatile(".byte 0x66; inl %%dx, %0" : "=a" (_tmp__) : "d" ((unsigned short)(port))); \
	_tmp__; })
...
({ unsigned char _tmp__; \
	__asm__ volatile("inb %%dx, %0" : "=a" (_tmp__) : "d" ((unsigned short)(port))); \
	_tmp__; })
...
({ __asm__ volatile("outl %0, %%dx" : : "a" (val) , "d" ((unsigned short)(port))); })
...
({__asm__ volatile(".byte 0x66; outl %0, %%dx" : : "a" ((unsigned short)(val)) , "d" ((unsigned short)(port))); })
...
({ __asm__ volatile("outb %0, %%dx" : : "a" ((unsigned char)(val)) , "d" ((unsigned short)(port))); })
