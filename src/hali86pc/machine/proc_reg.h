...
extern /* __inline */ unsigned get_eflags(void);
extern /* __inline */ void set_eflags(unsigned eflags);
extern /* __inline */ void cli(void);
extern /* __inline */ void sti(void);
extern /* __inline */ void cld(void);
#ifndef __cplusplus
extern /* __inline */ void std(void);
#endif
extern /* __inline */ void clts(void);
extern /* __inline */ unsigned short get_cs(void);
extern /* __inline */ unsigned short get_ds(void);
extern /* __inline */ void set_ds(unsigned short ds);
extern /* __inline */ unsigned short get_es(void);
extern /* __inline */ void set_es(unsigned short es);
extern /* __inline */ unsigned short get_fs(void);
extern /* __inline */ void set_fs(unsigned short fs);
extern /* __inline */ unsigned short get_gs(void);
extern /* __inline */ void set_gs(unsigned short gs);
extern /* __inline */ unsigned short get_ss(void);
...
#define get_eflags() \
    ({ \
	register unsigned int _temp__; \
	__asm__ volatile("pushfl; popl %0" : "=r" (_temp__)); \
	_temp__; \
    })
...
#define	get_cr0() \
    ({ \
	register unsigned int _temp__; \
	__asm__ volatile("mov %%cr0, %0" : "=r" (_temp__)); \
	_temp__; \
    })
...
#define	set_cr0(value) \
	...
	__asm__ volatile("mov %0, %%cr0" : : "r" (_temp__)); \
     })
...
#define get_cr4() \
    ({ \
	register unsigned int _temp__; \
	__asm__ volatile("mov %%cr4, %0" : "=r" (_temp__)); \
	_temp__; \
    })
#define set_cr4(value) \
    ({ \
	register unsigned int _temp__ = (value); \
	__asm__ volatile("mov %0, %%cr4" : : "r" (_temp__)); \
     })
#else /* not HAVE_CR4 */
#define get_cr4() \
    ({ \
	register unsigned int _temp__; \
	__asm__ volatile(".byte 0x0f,0x20,0xe0" : "=a" (_temp__)); \
	_temp__; \
    })
#define set_cr4(value) \
	__asm__ volatile(".byte 0x0f,0x22,0xe0" : : "a" (value));
#endif /* HAVE_CR4 */

#define get_msw() \
    ({ \
	unsigned short _msw__; \
	__asm__ volatile("smsw %0" : "=r" (_msw__)); \
	_msw__; \
    })
...
#define	set_cr3(value) \
    ({ \
	register unsigned int _temp__ = (value); \
	__asm__ volatile("mov %0, %%cr3" : : "r" (_temp__)); \
     })
...
#define	get_tr() \
	...
	__asm__ volatile("str %0" : "=rm" (_seg__) ); \
	_seg__; \
    })
...
#define set_gdt(pseudo_desc) \
    ({ \
	__asm__ volatile("lgdt %0" : : "m" ((pseudo_desc)->limit)); \
    })

#define set_idt(pseudo_desc) \
    ({ \
	__asm__ volatile("lidt %0" : : "m" ((pseudo_desc)->limit)); \
    })

#define	get_ldt() \
    ({ \
	unsigned short _seg__; \
	__asm__ volatile("sldt %0" : "=rm" (_seg__) ); \
	_seg__; \
    })
...
#define rdmsr(msr) \
    ({ \
	unsigned long low, high; \
	__asm__ volatile("rdmsr" : "=d" (high), "=a" (low) : "c" (msr)); \
	((unsigned long long)high << 32) | low; \
    })
#define wrmsr(msr, high, low) \
    ({ \
	__asm__ volatile("wrmsr" : : "d" (high), "a" (low), "c" (msr)); \
    })
#define wrmsrll(msr, val) \
    ({ \
	__asm__ volatile("wrmsr" : : "A" (val), "c" (msr)); \
    })
#else
#define get_tsc() \
    ({ \
...
#define rdmsr(msr) \
    ({ \
	unsigned long low, high; \
	__asm__ volatile(".byte 0x0f; .byte 0x32;" \
 	: "=d" (high), "=a" (low) : "c" (msr)); \
	((unsigned long long)high << 32) | low; \
    })
#define wrmsr(msr, high, low) \
    ({ \
	__asm__ volatile(".byte 0x0f; .byte 0x30" \
	: : "d" (high), "a" (low), "c" (msr)); \
    })
#define wrmsrll(msr, val) \
    ({ \
