...
#define pic_enable_all() ({		\
	_io_outb(MASTER_OCW, 0);		\
	_io_outb(SLAVES_OCW, 0);		\
})

#define pic_disable_all() ({		\
	_io_outb(MASTER_OCW, PICM_MASK);	\
	_io_outb(SLAVES_OCW, PICS_MASK);	\
})

#define pic_ack(irq) ({				\
	_io_outb(MASTER_ICW, NON_SPEC_EOI);		\
	if ((irq) >= 8)				\
		_io_outb(SLAVES_ICW, NON_SPEC_EOI);	\
})
