/*===---- printf.c - Print text to the screen or to a console -------------===
 *
...
static inline boolean_t _is_digit(char ch) {
  return (ch >= '0') && (ch <= '9');
}
...
static size_t _ntoa_format(out_fct_type out, char *buffer, size_t idx,
                           ...
                           boolean_t negative, unsigned int base,
...
static size_t _ntoa_long(out_fct_type out, char *buffer, size_t idx,
                         size_t maxlen, unsigned long value, boolean_t negative,
...
static size_t _ntoa_long_long(out_fct_type out, char *buffer, size_t idx,
                              ...
                              boolean_t negative, unsigned long long base,
                              unsigned int prec, unsigned int width,
                              unsigned int flags) {
...
static size_t _etoa(out_fct_type out, char *buffer, size_t idx, size_t maxlen,
  ...
  // determine the sign
  const boolean_t negative = value < 0;
  if (negative) {
    value = -value;
  }
...
#if defined(PRINTF_SUPPORT_LONG_LONG)
      const boolean_t is_ll = sizeof(uintptr_t) == sizeof(long long);
      if (is_ll) {
        idx = _ntoa_long_long(out, buffer, idx, maxlen,
